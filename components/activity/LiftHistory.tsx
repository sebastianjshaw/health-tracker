...
import { LiftEditSheet } from "./LiftEditSheet";
...
export function LiftHistory({ entries }: { entries: LiftHistoryEntry[] }) {
  const [editing, setEditing] = React.useState<LiftHistoryEntry | null>(null);
  ...
  return (
    <>
        ...
        {entries.map((entry) => (
          <Card key={entry.id} className="p-0">
            <button
              ...
              aria-label={`Edit workout ${entry.workout} on ${prettyDate(entry.date)}`}
            >
      ...
      <LiftEditSheet entry={editing} onClose={() => setEditing(null)} />
    </>
  );
}
